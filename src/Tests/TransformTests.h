...
TEST_CASE("Transform component works correctly") {
  ...
  SUB_GROUP("Transform forward points at target after LookAt") {
    ...
    VERIFY(modelSpaceTarget[X] < EPS && modelSpaceTarget[Y] < EPS && abs(modelSpaceTarget[Z] - target.Length()) < EPS)
  }
  ...
  SUB_GROUP("Follow moving target") {
    ...
    for (target = Vector3(0, 0, 2.6); target[Z] > -2.7; target -= Vector3(0, 0, 0.2)) {
      ...
      VERIFY(modelSpaceTarget[X] < EPS && modelSpaceTarget[Y] < EPS &&
             abs(modelSpaceTarget[Z] - (transform->position - target).Length()) < EPS)
    }
  }
}