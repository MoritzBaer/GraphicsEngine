...
template <typename T> inline MatrixT<4, 4, T> Perspective(T near, T far, T fov, T aspectRatio) {
  T s = static_cast<T>(tan(PI * (0.5 - fov / T(360))));
  T inv = far / (near - far);
...
template <typename T> MatrixT<3, 3, T> RodriguesRotation(VectorT<3, T> const &axis, T theta) {
  ...
  return MatrixT<3, 3, T>(
      cosTheta + a[X] * a[X] * (T(1) - cosTheta), -a[Z] * sinTheta + a[Y] * a[X] * (T(1) - cosTheta),
      a[Y] * sinTheta + a[Z] * a[X] * (T(1) - cosTheta), a[Z] * sinTheta + a[X] * a[Y] * (T(1) - cosTheta),
      cosTheta + a[Y] * a[Y] * (T(1) - cosTheta), -a[X] * sinTheta + a[Z] * a[Y] * (T(1) - cosTheta),
      -a[Y] * sinTheta + a[X] * a[Z] * (T(1) - cosTheta), a[X] * sinTheta + a[Y] * a[Z] * (T(1) - cosTheta),
...
template <typename T> MatrixT<4, 4, T> Rotate(VectorT<3, T> const &axis, T theta) {
  ...
  return MatrixT<4, 4, T>(R[0][0], R[0][1], R[0][2], 0, R[1][0], R[1][1], R[1][2], 0, R[2][0], R[2][1], R[2][2], 0, 0,
                          0, 0, 1);
}
...
template <typename T> MatrixT<4, 4, T> Rotate(VectorT<3, T> const &axis, VectorT<3, T> const &center, T theta) {
  ...
  return MatrixT<4, 4, T>(R[0][0], R[0][1], R[0][2], 0,
...
MatrixT<4, 4, T> LookAt(VectorT<3, T> const &eye, VectorT<3, T> const &target, VectorT<3, T> const &up) {
  ...
  return MatrixT<4, 4, T>(r[X], r[Y], r[Z], -(r * eye),
...
inline Vector3 RotateByQuaternion(Vector3 const &point, Quaternion const &rotation) {
  return (rotation * Quaternion(0, point) * rotation.Conjugate()).xyz();
