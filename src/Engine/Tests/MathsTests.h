...
TEST_ASSERT_EQUAL(float, v2, "own", glmv2, "glm", "GLM representation doesn't match internal representation!")
float v2l = v2.Length();
float glmv2l = glm::length(glmv2);
TEST_ASSERT_EQUAL(float, v2l, "own", glmv2l, "glm", "Length calculation gives wrong result!")

v2.Normalize();
TEST_ASSERT(abs(v2.Length() - 1) < EQUALITY_EPS, "Normalization results in incorrect length! ({})", v2.Length())
glmv2 = glm::normalize(glmv2);
TEST_ASSERT_EQUAL(float, v2, "own", glmv2, "glm", "Normalization gives incorrect result!")
...
TEST_ASSERT_EQUAL(float, axb, "own", glmaxb, "glm", "Cross product gives incorrect result!")
...
MatrixNM<2, 3> D = A * B;
TEST_ASSERT_EQUAL(float, D, "own", C, "expected", "Matrix multiplication does not give the correct result!")

Matrix4 M{1, 3, 2, 4, 0, 1, 1, 3, 2, 0, 0, 2, 1, 2, 4, 1};
Vector4 x{4, 2, 3, 0};
Vector4 y = (M * 2.5f) * (x * 2.5f);
Vector4 y_ = Vector4{10, 14, 10, 28} * (2.5f * 2.5f);
TEST_ASSERT_EQUAL(float, y, "own", y_, "expected", "Matrix-vector multiplication does not give the correct result!")

{
  Matrix2 M = Matrix2(1, 1, 2, 3);
  Matrix2 M_Inv = M.Inverse();
  Matrix2 M_Exp = Matrix2(3, -1, -2, 1);
  TEST_ASSERT_EQUAL(float, M_Inv, "own", M_Exp, "expected", "Matrix inversion does not give the correct result!")
}
{
  Matrix3 M = Matrix3(1, 2, 3, 4, 5, 6, 7, 8, 9);
  glm::mat3 glmM = glm::mat3(1, 4, 7, 2, 5, 8, 3, 6, 9);
  TEST_ASSERT_EQUAL(float, M, "own", glmM, "glm", "Matrix representation does not match for 3x3 matrices!")
...
TEST_ASSERT_EQUAL(float, m1, "own", glm1, "glm", "Matrix representation does not match!")
...
TEST_ASSERT_EQUAL(float, v2, "own", glmv2, "glm", "Matrix-vector multiplication does not give the correct result!")

Matrix4 m3 = m1 * m2;
glm::mat4 glm3 = glm1 * glm2;
TEST_ASSERT_EQUAL(float, m3, "own", glm3, "glm", "Matrix multiplication does not give the correct result!")
...
TEST_ASSERT_EQUAL(float, m_Inverse, "own", m_Expected, "expected",
                  "Matrix inversion does not give the correct result!");

END_TEST_CASE() // Matrix

BEGIN_TEST_CASE(transformation)
...
TEST_ASSERT_EQUAL(float, r1, "own", glmr1, "glm", "Rotation matrix is incorrect!")
...
TEST_ASSERT_EQUAL(float, l, "own", glm, "glm", "LookAt matrix is incorrect!")
...
TEST_ASSERT_EQUAL(float, p, "own", glmp, "glm", "Perspective matrix is incorrect!")

END_TEST_CASE() // transformation
...
TEST_ASSERT_EQUAL(float, cp, "quaternion", mp, "rodriguez", "Quaternion multiplication gives incorrect result!")
...
TEST_ASSERT_EQUAL(float, pp, "direct", pqr, "indirect",
