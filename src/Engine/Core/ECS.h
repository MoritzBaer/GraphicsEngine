...
#define ENGINE_COMPONENT_CONSTRUCTOR(name, ...) name(Engine::Core::_Entity e, __VA_ARGS__) : Engine::Core::Component<name>(e)
...
namespace Engine::Core
{
    ...
    public:
        ...
        template<class C>
        static void RegisterComponent();    // Components can only be registered after initialization
    ...
    class Entity {                          // Wrapper for internal entity, comfort only
        _Entity id;
    public:
        inline Entity(_Entity e) : id(e) { }
        inline Entity() : id(-1) { }
    ...
    template <class C>
    inline C * ECS::ComponentArray<C>::GetComponent(_Entity e)
    {
        return components[entityComponentIndexMap[e]];
    }

    template <class C>
    inline C * ECS::ComponentArray<C>::AddComponent(_Entity e)
    {
    ...
    template <class C>
    inline void ECS::ComponentArray<C>::RemoveComponent(_Entity e)
    {
    ...
    template <class C>
    inline C *ECS::AddComponent(_Entity e)
    {
        if (!(instance->aliveAndComponentFlags[e] & ALIVE_FLAG)) { ENGINE_ERROR("Tried to attach component to dead entity!") return nullptr; }
        if (HasComponent<C>(e)) { ENGINE_ERROR("Tried to attach same component twice!") return nullptr; }
    ...
    template <class C>
    inline C *ECS::GetComponent(_Entity e)
    {
        if (!(instance->aliveAndComponentFlags[e] & ALIVE_FLAG)) { ENGINE_ERROR("Tried to query component off dead entity!") return nullptr; }
        if (!HasComponent<C>(e)) { ENGINE_ERROR("Tried to query component the entity does not have!") return nullptr; }
        return reinterpret_cast<C *>(instance->componentArrays[C::COMPONENT_INDEX]->GetComponent(e));
    }

    template <class C>
    inline bool ECS::HasComponent(_Entity e)
    {
        return instance->aliveAndComponentFlags[e] & (COMPONENT_FLAG(C));
    }

    template <class C>
    inline void ECS::RemoveComponent(_Entity e)
    {
        if (!(instance->aliveAndComponentFlags[e] & ALIVE_FLAG)) { ENGINE_ERROR("Tried to query component off dead entity!") return nullptr; }
        if (!HasComponent<C>(e)) { ENGINE_ERROR("Tried to query component the entity does not have!") return nullptr; }
